            ...
            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 2rem;">
                <div>
                    ...
                    <div class="phases-list">
                        ${project.phases.length ? stats.phaseStats.map(p => this.createPhaseRow(project.id, p)).join('') : '<p>Žiadne fázy. Pridajte prvú pre sledovanie progresu.</p>'}
                    </div>
                </div>
                <div>
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                    ...
                    <div class="transactions-list card shadow" style="padding: 1.25rem;">
                        ${project.transactions.length ? project.transactions.slice().reverse().map(t => this.createTransactionRow(project.id, t)).join('') : '<p style="color: var(--text-muted); font-size: 0.875rem;">Žiadne výdavky.</p>'}
                    </div>
                </div>
            </div>
        `;
    ...
    createPhaseRow(projectId, phase) {
        ...
        const taskHtml = (phase.tasks || []).map(t => `
            <div style="display: flex; align-items: center; gap: 0.5rem; margin-top: 0.5rem;">
                ...
                <span style="flex: 1; font-size: 0.875rem; ${t.completed ? 'text-decoration: line-through; color: var(--text-muted);' : ''}">${t.text}</span>
                ...
                <div style="display: flex; align-items: center; gap: 1.25rem; margin-bottom: 1rem;">
                    <div style="flex: 1; height: 12px; background: #f3f4f6; border-radius: 6px; overflow: hidden; position: relative;">
                         <input type="range" value="${phase.progress}" onchange="App.updatePhaseProgress('${projectId}', '${phase.id}', this.value)" 
                            ...
                            ${hasTasks ? 'disabled' : ''}>
                         <div style="width: ${phase.progress}%; height: 100%; background: var(--primary); transition: width 0.3s;"></div>
                    </div>
                    <span style="width: 45px; text-align: right; font-size: 0.9rem; font-weight: 700; color: var(--primary);">${phase.progress}%</span>
